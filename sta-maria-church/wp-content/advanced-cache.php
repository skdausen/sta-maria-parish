...
// WP-Optimize advanced-cache.php (written by version: 4.2.2) (homeurl: https://192.168.1.4/sta-maria-church/) (abspath: C:\xampp\htdocs\sta-maria-church/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'C:\xampp\htdocs\sta-maria-church\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://192.168.1.4/sta-maria-church/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\xampp\htdocs\sta-maria-church\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-192.168.1.4.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-192.168.1.4.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-192.168.1.4.php');
}
